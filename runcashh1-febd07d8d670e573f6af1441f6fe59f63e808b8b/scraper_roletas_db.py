...
# Função para inicializar o adaptador do novo banco de roletas
def inicializar_adaptador_roletas():
    """Inicializa o adaptador para o novo banco de roletas"""
    if not ADAPTADOR_ROLETAS_DISPONÍVEL:
        print("Adaptador para novo banco de roletas não disponível!")
        return None
    
    try:
        print("Inicializando adaptador para novo banco de roletas...")
        adaptador = ScraperAdapter()
        return adaptador
    except Exception as e:
        print(f"Erro ao inicializar adaptador: {str(e)}")
        return None

# Variável global para o adaptador
adaptador_roletas = inicializar_adaptador_roletas()
...
# Importações para novo banco de dados de roletas
try:
    from adaptar_scraper_roletas_db import ScraperAdapter
    ADAPTADOR_ROLETAS_DISPONÍVEL = True
    print("Adaptador para novo banco de roletas importado com sucesso!")
except ImportError as e:
    print(f"Aviso: {e}")
    print("Executando sem o adaptador para novo banco de roletas.")
    ADAPTADOR_ROLETAS_DISPONÍVEL = False
...
# Wrapper para compatibilidade com o novo banco de roletas
def scrape_roletas_wrapper(db, driver=None, numero_hook=None):
    """Wrapper que decide qual versão do scraper usar baseado no db"""
    global adaptador_roletas
    
    # Se db é o adaptador ou o adaptador não está disponível, usar o scraper original
    if not ADAPTADOR_ROLETAS_DISPONÍVEL or db is adaptador_roletas:
        print("Usando scraper original com o banco de dados fornecido")
        return scrape_roletas_original(db, driver, numero_hook)
    
    # Se o adaptador está disponível, substituir db pelo adaptador
    if adaptador_roletas is not None:
        print("Usando scraper com o adaptador para o novo banco de roletas")
        return scrape_roletas_original(adaptador_roletas, driver, numero_hook)
    else:
        print("Adaptador não inicializado, usando scraper original")
        return scrape_roletas_original(db, driver, numero_hook)

# Renomear a função original
scrape_roletas_original = scrape_roletas
# Substituir pela função wrapper
scrape_roletas = scrape_roletas_wrapper
...
# Arquivo adaptado para uso com o novo banco de dados de roletas - 2025-05-06 21:00:04
