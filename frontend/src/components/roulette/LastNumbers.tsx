...
const LastNumbers = memo(({ numbers, isLoading }: LastNumbersProps) => {
  ...
  useEffect(() => {
    ...
    console.log('[LastNumbers] Números válidos:', validNumbers.length);
    console.log('[LastNumbers] Tipo de numbers:', typeof numbers, Array.isArray(numbers));
    
    if (newNumberIndex === 0) {
      console.log('[LastNumbers] Novo número detectado:', validNumbers[0]);
    }
  }, [validNumbers, isLoading, newNumberIndex, numbers]);
  ...
  return (
    <div className="h-[74px] flex flex-wrap content-start gap-1 my-2 w-full overflow-hidden" data-testid="last-numbers">
      {validNumbers.slice(0, 12).map((num, idx) => {
        console.log(`[LastNumbers] Renderizando número ${num} na posição ${idx}`);
        return (
          <div
            key={`${num}-${idx}`}
