...
export function useRouletteData(
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        debugLog(`[useRouletteData] Resposta da API para ${roletaNome}:`, numerosArray);
        ...
        if (numerosArray && numerosArray.length > 0) {
          ...
          debugLog(`[useRouletteData] Números formatados para ${roletaNome}:`, formattedNumbers);
          ...
          // Notificar o usuário que não há dados disponíveis
          toast({
  ...
  const handleNewNumber = useCallback((event: RouletteNumberEvent) => {
    ...
    setNumbers(prev => {
      ...
      debugLog(`[useRouletteData] Adicionando novo número para ${roletaNome}:`, newNumber);
      setHasData(true);
      
      // Limitar ao número máximo de elementos
      const updatedNumbers = [newNumber, ...prev].slice(0, limit);
      debugLog(`[useRouletteData] Lista atualizada para ${roletaNome}:`, updatedNumbers.slice(0, 3));
  ...
  useEffect(() => {
    debugLog(`[useRouletteData] Estado atual para ${roletaNome}:`, {
      hasData,
      numbersCount: numbers.length,
      isConnected,
      loading
    });
  }, [numbers, hasData, isConnected, loading, roletaNome]);
  
  // Adicionar um efeito para forçar a atualização dos dados a cada 10 segundos
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      debugLog(`[useRouletteData] Atualizando dados automaticamente para ${roletaNome}`);
      refreshNumbers();
    }, 10000);
