...
class GlobalRouletteDataService {
  ...
  /**
   * Atualiza os dados de uma roleta específica
   * @param updatedRoulette A roleta com dados atualizados
   */
  public updateRoulette(updatedRoulette: any): void {
    if (!updatedRoulette || !updatedRoulette.id) {
      console.error('[GlobalRouletteService] Tentativa de atualizar roleta sem ID');
      return;
    }
    
    try {
      console.log(`[GlobalRouletteService] Atualizando roleta: ${updatedRoulette.nome || updatedRoulette.name || updatedRoulette.id}`);
      
      // Identificadores possíveis da roleta
      const roletaId = updatedRoulette.id || updatedRoulette._id;
      const roletaNome = updatedRoulette.nome || updatedRoulette.name;
      
      // Atualizar nos dados normais
      if (this.rouletteData && Array.isArray(this.rouletteData)) {
        const index = this.rouletteData.findIndex(r => 
          r.id === roletaId || 
          r._id === roletaId ||
          (roletaNome && (r.nome === roletaNome || r.name === roletaNome))
        );
        
        if (index >= 0) {
          console.log(`[GlobalRouletteService] Roleta encontrada nos dados normais, atualizando índice ${index}`);
          this.rouletteData[index] = {
            ...this.rouletteData[index],
            ...updatedRoulette
          };
        }
      ...
      // Atualizar nos dados detalhados se existirem
      if (this.detailedRouletteData && Array.isArray(this.detailedRouletteData)) {
        const detailedIndex = this.detailedRouletteData.findIndex(r => 
          r.id === roletaId || 
          r._id === roletaId ||
          (roletaNome && (r.nome === roletaNome || r.name === roletaNome))
        );
        ...
        if (detailedIndex >= 0) {
          console.log(`[GlobalRouletteService] Roleta encontrada nos dados detalhados, atualizando índice ${detailedIndex}`);
          this.detailedRouletteData[detailedIndex] = {
            ...this.detailedRouletteData[detailedIndex],
            ...updatedRoulette
          };
        }
      ...
      console.log(`[GlobalRouletteService] Roleta ${roletaId} atualizada com sucesso`);
    } catch (error) {
      console.error('[GlobalRouletteService] Erro ao atualizar roleta:', error);
    }
  }
}
