...
import StrategyDataPanel from '@/components/mongodb/StrategyDataPanel';
  ...
  const fetchRoulettes = async () => {
    try {
      ...
      if (allowedData.length === 0) {
        ...
        // FALLBACK: Usar dados simulados quando não há roletas do backend
        console.log('[DEBUG] Usando dados simulados como fallback...');
        const simulatedData = [
          { id: "2010016", nome: "Immersive Roulette", vitorias: 0, derrotas: 0 },
          { id: "2380335", nome: "Brazilian Mega Roulette", vitorias: 0, derrotas: 0 },
          { id: "2010065", nome: "Bucharest Auto-Roulette", vitorias: 0, derrotas: 0 },
          { id: "2010096", nome: "Speed Auto Roulette", vitorias: 0, derrotas: 0 },
          { id: "2010017", nome: "Auto-Roulette", vitorias: 0, derrotas: 0 },
          { id: "2010098", nome: "Auto-Roulette VIP", vitorias: 0, derrotas: 0 }
        ];
        
        // Formatar dados simulados
        const formattedSimulatedData = simulatedData.map(item => ({
          id: item.id,
          name: item.nome,
          roleta_nome: item.nome,
          wins: item.vitorias || 0,
          losses: item.derrotas || 0
        }));
        
        console.log('[DEBUG] Usando dados simulados:', formattedSimulatedData);
        setRoulettes(formattedSimulatedData);
        
        // Notificar usuário sobre dados simulados
        toast({
          title: "Usando dados de demonstração",
          description: "Não foi possível carregar roletas do servidor. Exibindo dados de demonstração.",
          variant: "default",
          duration: 5000
        });
      } else {
      ...
      // FALLBACK: Usar dados simulados em caso de erro
      console.log('[DEBUG] Erro ao buscar roletas, usando dados simulados...');
      const simulatedData = [
        { id: "2010016", nome: "Immersive Roulette", vitorias: 0, derrotas: 0 },
        { id: "2380335", nome: "Brazilian Mega Roulette", vitorias: 0, derrotas: 0 },
        { id: "2010065", nome: "Bucharest Auto-Roulette", vitorias: 0, derrotas: 0 },
      ...
      toast({
        ...
        description: "Usando dados de demonstração. Conexão com o servidor falhou.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
