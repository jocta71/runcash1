 ...
 * Adiciona req.subscription com os dados da assinatura se existir
 */
const checkSubscription = async (req, res, next) => {
  ...
  try {
    ...
    // Buscar dados de assinatura do usuário no MongoDB
    const userSubscription = await db.collection('user_subscriptions').findOne({
      userId: req.user._id.toString()
    });
    
    // Se não tiver assinatura no banco, não tem acesso
    if (!userSubscription) {
    ...
    if (userSubscription.status === 'ACTIVE') {
      ...
      if (expiresAt > now) {
        // Assinatura válida, adicionar à requisição
    ...
    console.error('Erro ao verificar assinatura:', error);
    // Em caso de erro, prosseguir sem assinatura
    req.subscription = null;
    next();
  }
};
...
/**
 * Middleware para exigir assinatura ativa para acessar a rota
 */
const requireSubscription = (req, res, next) => {
  ...
  if (!req.subscription) {
    return res.status(403).json({
      success: false,
      message: 'Esta funcionalidade requer um plano ativo',
