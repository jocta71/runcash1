...
// Adicionar endpoint SSE /api/stream/roulettes
app.get('/api/stream/roulettes', (req, res) => {
  ...
  if (!isConnected || !collection) {
    ...
    console.log(`[SSE ${requestId}] MongoDB não conectado, enviando erro ao cliente`);
  } else {
    // Enviar dados iniciais das roletas
    (async () => {
      try {
        // Buscar os números mais recentes para cada roleta
        const results = await collection.aggregate([
        ...
        if (results.length === 0) {
          console.log(`[SSE ${requestId}] Nenhum número encontrado na coleção`);
          sendEvent('update', []);
        } else {
    ...
    // Criar um intervalo dedicado para cada cliente SSE
    const clientInterval = setInterval(async () => {
      try {
        ...
        // Buscar atualizações mais recentes
        const results = await collection.aggregate([
          { $sort: { timestamp: -1 } },
          { $group: { 
              _id: '$roleta_nome', 
        ...
        console.error(`[SSE ${requestId}] Erro durante SSE polling:`, error);
        sendEvent('error', {
          error: true,
    ...
    req.on('close', () => {
      console.log(`[SSE ${requestId}] Conexão fechada pelo cliente`);
